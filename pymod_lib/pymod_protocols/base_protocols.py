...
class AFDB_common:
    ...
    def __init__(self, parent, id):
        ...
        self.uniprot_header = self.get_uniprot_header(id)

    def init_run_AFDB_uniprot_id_search(self):

        if self.uniprot_header is not None:
            self.af_model_id = self.run_AFDB_uniprot_id_search(self.uniprot_header)

    def connect_to_afdb(self, output_dir):

        ##### MODIFIED #####
        if not check_network_connection("https://google.com", timeout=3):
            title = "Connection Error"
            message = ("An internet connection is not available, can not connect to the AlphaFold database")
        ...
        full_link = link_to_afdb_api + str(self.uniprot_header)

        # Open link and get data in JSON format
        response = urllib.request.urlopen(full_link)
        data = json.loads(response.read())

        # Get url, from the JSON data, to download the '.pdb' file of the AlphaFold model
        url_to_pdb_download = data[0]["pdbUrl"]

        # Download AlphaFold model in pdb format
        response = requests.get(url_to_pdb_download)

        self.output_dir = output_dir
        self.full_output_path = os.path.join(self.output_dir, "AF-" + self.uniprot_header + ".pdb")

        file = open(self.full_output_path, "wb")
    ...
    def get_uniprot_header(self, id):
        ...
        uniprot_id = re.search("[OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z][0-9]([A-Z][A-Z0-9]{2}[0-9]){1,2}", title)

        ##### MODIFIED #####
        if uniprot_id is not None:
        #if str(uniprot_id.group(0)):
        ##### END #####
            return str(uniprot_id.group(0))
        else:
            return None


    def run_AFDB_uniprot_id_search(self, uniprot_header):
        ...
        try:
            ...
            # Get AlphaFold model ID from the JSON data.
            #TODO: SEE WHAT HAPPENS IF ENTRYid IS NOT FOUND
            af_model_id = data[0]["entryId"]

        except:
            af_model_id = "Not Found"
