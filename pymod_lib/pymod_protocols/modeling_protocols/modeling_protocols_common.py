...
from pymod_lib.pymod_protocols.base_protocols import PyMod_protocol, AFDB_common
...
class Fetch_models_file(PyMod_protocol):
    """
    Class for downloading 3D-structures files of the models.
    """

    protocol_name = "fetch_models_files"
    ...
    def launch_from_gui(self):
        ...
        self.fetch_models_files()


    def fetch_models_files(self):
        """
        This method will call other methods to fetch the models files and load the corresponding
        elements in PyMod.
        """
        ...
        try:
            if not self.pymod.use_protocol_threads:
                self.download_all_models_files()
            else:
                label_text = ("Connecting to the AFDB to download %s. Please wait for"
                ...
                p_dialog = Protocol_exec_dialog(app=self.pymod.main_window, pymod=self.pymod,
                                                function=self.download_all_models_files,
        ...
        if n_failures != 0:
            ...
            ##### MODIFIED #####
            message = ("Can not access the %s database to download %s structures out of %s."
                       "\nPlease check your Internet connection or if the PDB ids of the"
                       " structures are valid." % ("AF", n_failures, len(self.structures_to_fetch)))
            ##### END #####
            ...
            if not fetch_results["failure"]:
                try:
                    ...
                    message = ("The structure association for %s chain %s failed because"
                               " of the following error: %s" % (fetch_results["model_id"],
                                                                fetch_results["model_chain"],
                                                                str(e)))
    ...
    def download_all_models_files(self):
        """
        Downloads all the structure files from the Database (AlphaFold DB or ESMFold DB).
        """
    ...
    def _fetch_single_element(self, old_element):
        ...
        """
        model_code = str(old_element.model_id)
        fetch_results = {"model_id": model_code,
                         "element": old_element}
        try:
            model_file_shortcut = fetch_models_file(model_id=model_code,
                output_dir=self.pymod.temp_directory_dirpath)
            fetch_results["failure"] = False
            fetch_results["model_file_shortcut"] = model_file_shortcut
        except IOError:
    ...
    def get_model_chain(self, model_file_shortcut, old_element):

        parsed_file_handle = open(model_file_shortcut, "r")

        parsed_biopython_structure = PDBParser(PERMISSIVE=1, QUIET=True).get_structure("temp", parsed_file_handle)
        parsed_file_handle.close()

        for model in parsed_biopython_structure.get_list():
            for chain in model.get_list():
                old_element.model_chain = str(chain.id)
    ...
    def _load_structure(self, fetch_results):
        ...
        model_id = fetch_results["model_id"]
        model_file_shortcut = fetch_results["model_file_shortcut"]

        self.get_model_chain(model_file_shortcut, old_element)

        model_chain_id = str(old_element.model_chain)

        fetch_results["model_chain"] = model_chain_id
        model_chain = fetch_results["model_chain"]
        ...
        if self.import_mode == "single-chain":
            ...
            a.associate(model_file_shortcut, model_chain)
        ...
        #----------------------------------------------------------------------
            ...
            a = Associate_structure(self.pymod, old_element, ASSOCIATE_ALL = True)
            a.associate(model_file_shortcut, model_chain)

        new_element = a.return_new_element()
        self.new_element_with_structure = new_element

        ##### MODIFIED #####
        #self.pymod.af_modeling_session_list[self.pymod.current_af_session]["selected_models"] = {}
        #self.pymod.af_modeling_session_list[self.pymod.current_af_session]["selected_models"].append(self.new_element_with_structure)
        #self.pymod.af_modeling_session_list[self.pymod.current_af_session][self.new_element_with_structure] = str(self.new_element_with_structure).rsplit(' ', 1)[0]
        self.pymod.af_modeling_session_list[self.pymod.current_af_session][str(self.new_element_with_structure).rsplit(' ', 1)[0]] = {"af_session_dirpath": self.pymod.af_session_dirpath,
                                                                                                                                    "model_element": self.new_element_with_structure}

        print(self.pymod.af_modeling_session_list)

        # self.pymod.af_modeling_session_list[self.pymod.current_af_session]["af_model_object"] = self.new_element_with_structure
        # ##### END #####
            ...
            # new_elements = self.pymod.open_structure_file(os.path.abspath(model_file_shortcut))
            # # Color other chains by gray.
            # other_chains_elements = [e for e in new_elements if e.get_chain_id() != model_chain]
            # if other_chains_elements:
            #     self.pymod.main_window.color_selection("multiple", other_chains_elements, "regular", regular_color="gray")
            #


class Structure_model_file_fetcher:
    """
    Class to fetch a PDB file from the AFDB.
    """

    def __init__(self, model_id, output_dir, new_name=None):
        ...
        if new_name:
            ...
            self.output_name = '%s.pdb' % model_id


    def fetch(self):
        """
        Actually retrieves the model files from the internet.
        """

        afdb_init = AFDB_common(self, self.model_id)
        afdb_search = afdb_init.connect_to_afdb(self.output_dir)

        return afdb_search

def fetch_models_file(model_id, output_dir, new_name=None):
    sf = Structure_model_file_fetcher(model_id, output_dir, new_name=new_name)
    return sf.fetch()
...
class Associate_structure(PyMod_protocol):
    ...
    def __init__(self, pymod, pymod_element, ASSOCIATE_ALL = False, permissive=True):
        ...
        self.ASSOCIATE_ALL = ASSOCIATE_ALL


    def associate(self, model_file_path, model_chain):
        """
        ...
        # ##### MODIFIED #####
        # self.pymod.af_modeling_session_list[self.pymod.current_af_session]["af_session_dirpath"] = self.af_session_dirpath
        # ##### END #####

        if os.path.isdir(self.af_session_dirpath):
            pass
        else:
            os.mkdir(self.af_session_dirpath)

        # Parses the source structure file.
        if not self.associate_pdb_file:
            self.associate_pdb_file = Parsed_pdb_file(self.pymod, self.original_model_file_path,
                                                      copy_original_file=False, save_chains_files=False)

        #-----------------------------------------------------------------------
        # Check if the pymod element can be associated to the structure chain. -
        #-----------------------------------------------------------------------
        if not self.model_chain in self.associate_pdb_file.get_chains_ids():
            raise KeyError("The structure file '%s' does not have chain '%s'." % (self.original_model_file_path, self.model_chain))

        structure_chain_element = self.associate_pdb_file.get_pymod_element_by_chain(self.model_chain)
        ...
        if not self.ASSOCIATE_ALL:
            ...
            cmd.load(self.original_model_file_path, self.temp_full_name)
            # Select amino acid residues ranging from the starting and ending positions which define
            # the fragment to "excise".
            cmd.select(self.temp_fragment_name, "resi %s-%s and chain %s and object %s and not hetatm" % (start_position, end_position, self.model_chain, self.temp_full_name))
            # Join the selections and save a file PDB file of the cropped fragment.
            pdb_filename = os.path.splitext(os.path.basename(self.original_model_file_path))[0] # structure_chain_element.get_structure_file_root()
            ...
            p = Parsed_pdb_file(self.pymod, os.path.abspath(cropped_structure_file_shortcut),
                                output_directory=self.af_session_dirpath)
            self.new_element_with_structure = p.get_pymod_element_by_chain(self.model_chain)
            adjust_sequence = self.target_element.my_sequence
            self.pymod.replace_element(self.target_element, self.new_element_with_structure)
            ...
            # plugin crash).
            if self.new_element_with_structure.my_sequence.replace("-", "") != new_sequence.replace("-", ""):
                ali_new = global_pairwise_alignment(adjust_sequence,
                                                    self.new_element_with_structure.my_sequence,
                                                    toss_modres=True)
                new_sequence = ali_new["seq2"]

            self.new_element_with_structure.set_sequence(new_sequence, permissive=False)
            ...
            # Builds a 'Parsed_pdb_file' object for the PDB file of the structure just saved. -
            #----------------------------------------------------------------------------------
            p = Parsed_pdb_file(self.pymod, os.path.abspath(self.original_model_file_path),
                                output_directory=self.af_session_dirpath)
            self.new_element_with_structure = p.get_pymod_element_by_chain(self.model_chain)
            adjust_sequence = self.target_element.my_sequence
            self.pymod.replace_element(self.target_element, self.new_element_with_structure)
    ...
    def return_new_element(self):
        return self.new_element_with_structure

    def _set_options(self, model_file_path, model_chain):

        self.original_model_file_path = model_file_path

        name = os.path.basename(os.path.normpath(model_file_path))

        self.model_file_path = os.path.join(self.temp_directory, name)
