...
class Generic_BLAST_search(PyMod_protocol):
    ...
    def launch_from_gui(self, AF = False):
        ...
        self.AF = AF
        if AF:
            self.build_blast_window_for_AFDB_search()
        else:
    ...
    def build_blast_window_for_AFDB_search(self):
        """
        Builds a window containing the widget necessary to define the options for BLAST and
        PSI-BLAST searches.
        """

        blast_window_class_qt = self.get_blast_window_class_qt(AF = True)
        self.blast_options_window = blast_window_class_qt(self.pymod.main_window, protocol=self,
            title= "Local BLAST Search - Options",
            upper_frame_title="Here you can modify options for %s" % (self.blast_version_full_name),
            submit_command=self.blast_window_state)
        self.blast_options_window.show()
    ...
    def check_blast_results(self):
        ...
        if self.AF:
            label_text = "Running AFDB search. Please wait for the process to complete..."
            p_dialog = Protocol_exec_dialog(app=self.pymod.main_window, pymod=self.pymod,
                                            function=self.build_hsp_list,
                                            args=(),
                                            title="Running AFDB search",
    ...
    def get_hsp_info(self, hsp, full_query_sequence = None):
        ...
        # Gets the AlphaFold model associated with the Uniprot-ID
        if self.AF:
            af_mod_init = AFDB_common(self, hsp)
            af_mod_id_search = af_mod_init.init_run_AFDB_uniprot_id_search()
            af_mod = af_mod_init.af_model_id
    ...
    def build_hsp_list(self):
        ...
        for alignment in self.search_record.alignments:
            for hsp in alignment.hsps:
                ...
                hsp_list.append(hsp)
        # Sort by evalue.
        self.hsp_list = list(sorted(hsp_list, key=lambda h: h.expect))
        return self.hsp_list
    ...
    def build_hits_to_import_list(self):
        ...
        for hsp in self.hsp_list:
            ...
            if self.my_blast_map[self.total_hsp_counter] == 1:
                ...
                if self.AF:
                    self.hsp_imported_from_blast.append({"hsp": hsp, "title": hsp.pymod_info["af_model"]})
                else:
    ...
    def import_results_in_pymod(self):
        ...
        if self.AF:
            ...
            # ##### MODIFIED #####
            # self.pymod.af_modeling_session_list[self.pymod.current_af_session]["import_mode_choice"] = self.blast_output_window.import_mode_cb.currentText()
            # ##### END #####

            # The user is asked whether to import the whole structure or just the hit fragment
            if self.blast_output_window.import_mode_cb.currentText() == "Whole Structure":
                import_mode = "multiple-chains"
            else:
                import_mode = "single-chain"

            self.blast_output_window.destroy()

            for pymod_elements in hsp_elements:
                # Download AF model
                fp = Fetch_models_file(self.pymod)
                fp.initialize_from_gui("single", pymod_elements)
                fp.import_mode = import_mode
...
class Similarity_searches_results_window_qt(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent, protocol, AF):
        ...
        if AF:
            ...
            # The download of the 3D-structure is automatically triggered after having checked the BLAST outputs of interest
            ...
            self.buttons_hbox.addWidget(self.import_mode_label)
            self.buttons_hbox.addWidget(self.import_mode_cb)
        ...
        # Layout
        self.buttons_hbox.addWidget(self.searchbar)
        ##### END #####
    ...
    def display_blast_hits(self):
        ...
        if self.protocol.AF:
            self.af_model_label = QtWidgets.QLabel("AlphaFold model")
            self.af_model_label.setStyleSheet(headers_font_style)
            self.results_grid.addWidget(self.af_model_label, 0, 5)
        ...
        ##### MODIFIED #####
        self.subject_checkbox_list = []
        ##### END #####

        for num, hsp in enumerate(self.protocol.hsp_list):

            ##### MODIFIED #####
            # If the identity is 100%, highlight the result
            identity = "%.1f%%" % (hsp.pymod_info["seqid"]*100)
            #print(identity)
            if identity == "100.0%":
                _font_style = small_font_style_colored
            else:
                _font_style = small_font_style
            ##### END #####

            # Hit name and checkbox.
            subject_name = self.protocol.get_subject_name(hsp)
            hsp_checkbox = QtWidgets.QCheckBox(subject_name)
            hsp_checkbox.setStyleSheet(_font_style)
            self.sbjct_checkbuttons_list.append(hsp_checkbox)

            ##### MODIFIED #####
            self.subject_checkbox_list.append((subject_name, hsp_checkbox))
            ##### END #####
            ...
            evalue_label.setStyleSheet(_font_style)
            ...
            if self.protocol.protocol_name != "hmmsearch":
                ...
                identities_label.setStyleSheet(_font_style)
            ...
            span_info_label.setStyleSheet(_font_style)
            ...
            hspan_info_label.setStyleSheet(_font_style)
            self.results_grid.addWidget(hspan_info_label, self.blast_output_row, 4)

            # AlphaFold model info.
            if self.protocol.AF:
                af_info_text = "%s" % (hsp.pymod_info["af_model"])
                af_info_label = QtWidgets.QLabel(af_info_text)
                af_info_label.setStyleSheet(_font_style)
                self.results_grid.addWidget(af_info_label, self.blast_output_row, 5)
    ...
    def update_display(self):
        num_checked_checkboxes = 0
        for subject, checkbox in self.subject_checkbox_list:
            if self.searchbar.text().lower() in subject.lower():
                headers_font_style = "%s; color: %s; font-weight: bold" % (small_font_style, highlight_color)
                checkbox.setStyleSheet(headers_font_style)
                num_checked_checkboxes += 1
            else:
                headers_font_style = "%s; color: %s" % (small_font_style, "#ffffff")
                checkbox.setStyleSheet(headers_font_style)

        if not num_checked_checkboxes:
            #print(dir(self.protocol))
            title = 'Search Error'
            message = 'The keyword typed in the search bar could not be found within the BLAST results.'
            self.protocol.pymod.main_window.show_error_message(title, message)
    ##### END #####
