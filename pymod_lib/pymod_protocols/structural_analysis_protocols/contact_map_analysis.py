...
class Contact_map_analysis(PyMod_protocol, Structural_analysis_mixin):
    ...
    def contact_map_analysis_state(self):
        ...
        ##### MODIFIED #####
            ...
            title = "Predicted Aligned Error (PAE) Map."
        ##### END #####
...
class Contact_map_options_window_qt(PyMod_protocol_window_qt):
    ...
    """
    ##### MODIFIED #####
    feature_types_list = ["Contacts", "Distances", "Distance Difference", "Distance Mean", "Distance Std", "PAE"]
    ##### END #####
    ...
    def get_feature_type(self):
        ...
        ##### MODIFIED #####
        elif feature_type == self.feature_types_list[5]:
            return "pae", feature_type
        ##### END #####
        else:
...
class Contact_map_analysis_window_qt(QtWidgets.QMainWindow):
    ...
    def initialize_map(self, pymod, data_array,
        ...
        ##### MODIFIED #####
        if self.feature_type in ("contact", "distance", "pae"):
    ...
    def draw_map(self):
        ...
        ##### MODIFIED #####
        elif self.feature_type in ("distance", "pae", "distances_mean"):
        ##### END #####
        ...
        ##### MODIFIED #####
            ...
            # Draws the map of a single structure on the canvas.
            self.canvas_plot_move_event = self.move_on_pae_plot
            self.canvas_plot_left_click_event = self.click_on_plot
        ##### END #####
    ...
    ##### MODIFIED #####
    def move_on_pae_plot(self, i, j):
        """
        Used when showing the PAE map of a single structure.
        """
        val = self.data_array[i][j]
        res_1 = self.ref_residues[i]
        res_2 = self.ref_residues[j]
        self.view_label.setText("Couple: %s %s - %s %s (PAE = %s)." % (res_1.three_letter_code, res_1.db_index,
                                                                 res_2.three_letter_code, res_2.db_index,
                                                                 self.get_value_label(val)) + "\n"
                                                                 + "Click on the plot to view the distance between the two residues in PyMOL.")
    ##### END #####
