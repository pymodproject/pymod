...
class Regular_alignment(Alignment_protocol):
    ...
    def align_and_keep_previous_alignment(self):
        ...
        print("self.involved_clusters_list")
        print(self.involved_clusters_list)
        ...
        print("anchor_seq_in-cluster")
        print(anchor_seq_in_cluster)
        print("external_seq")
        print(external_seq)
        print("[r for r in c0_recs if r.id != anchor_seq.id]")
        print([r for r in c0_recs if r.id != anchor_seq.id])
        aligned_pairs = []
        print("all_recs")
        print(all_recs)
        ...
        for rec_i_id, rec_i in enumerate(all_recs):
            for rec_j in all_recs[rec_i_id:]:
                ...
                if rec_i.id == anchor_seq.id:
                    print("if rec_i.id == anchor_seq.id:")
                    print(rec_i.id, anchor_seq.id)
    ...
    def create_alignment_element(self, AF = False):
        """
        ...
        if self.alignment_mode == "build-new-alignment":
            ...
            self.alignment_element = self.pymod.add_new_cluster_to_pymod(cluster_type=cluster_type,
