...
class Alignment_protocol(PyMod_protocol):
    ...
    #################################################################
    ################## New in Pymod - 2023 ##########################
    # The 'launch_no_gui' and 'initialize_alignment_no_gui' method  #
    # can be used for performing an alignment when the sequences    #
    # to be aligned are not initialized from the GUI                #
    #################################################################

    def launch_no_gui(self):
        if self.alignment_program_exists():
            self.initialize_alignment_no_gui()
        else:
    ...
    def initialize_alignment_no_gui(self):
        ...
        self.create_alignment_element(AF = True)
