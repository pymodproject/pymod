...
##### MODIFIED #####
class Uniprot_id_window_qt(PyMod_tool_window_qt):
    """
    Window with one text entry for the UniProt ID of the sequence element to add to PyMod.
    """

    build_uniprot_id_entry = True
    ...
    def add_middle_frame_widgets(self):
        ...
        # Creates an Entry for the name of the new sequence.
        if self.build_uniprot_id_entry:
            self.uniprot_id_input = PyMod_entryfield_qt(label_text="UniProt ID:", value="", style=entry_style)
            self.middle_formlayout.add_widget_to_align(self.uniprot_id_input)

    def get_sequence_uniprot_id(self):
        return self.uniprot_id_input.getvalue()
##### END #####
