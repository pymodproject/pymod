...
class PyMod_elements_interactions:
    ...
    def add_new_cluster_to_pymod(self, cluster_type="generic",
        ...
        if not cluster_type in ("alignment", "blast-cluster", "profile-cluster", "af-cluster", "generic"):
        ...
        elif cluster_type == "af-cluster":
            self.af_cluster_counter += 1
        ...
        if cluster_name == None:
            if cluster_type == "alignment":
                ...
                cluster_name = "AlphaFold cluster %s" % (self.af_cluster_counter)
    ...
    def all_sequences_have_fetchable_af_models(self, selection=None):
        """
        Returns 'True' if all the elements in the selection can be used to download a AlphaFold model.
        """
        selection = self.build_sequence_selection(selection)
        return self.check_all_elements_in_selection(selection, "af_model_is_fetchable")
    ...
    def get_header(self, pymod_element):

        return seq_headers.get_header_string(pymod_element.original_header)
    ...
    def set_structure_header(self, pymod_element, full_structure_name=None, chain_file_name=None):
        ...
        # If the PDB structure is a model, the associated file is saved in the 'Models' dir (e.g. AlphaFold structures), otherwise in the 'Structures' dir
        if pymod_element.my_header_root.startswith("AF-"):
            self.af_session_dirpath = os.path.join(self.models_dirpath, pymod_element.my_header_root.split("_")[0] + "-session")
            pathto = self.af_session_dirpath
        else:
            pathto = self.structures_dirpath

        # Renames the chain file.
        renamed_chain_str_file = os.path.join(pathto, "%s%s.pdb" % (pymod_element.compact_header_prefix, pymod_element.my_header_root))
        ...
        # If the PDB structure is a model, after being saved in the 'Models' dir, it is also copied in the 'Structures' dir
        if pymod_element.my_header_root.startswith("AF-"):
            path_to_structures = os.path.join(self.structures_dirpath, "%s%s.pdb" % (pymod_element.compact_header_prefix, pymod_element.my_header_root))
            shutil.copy(renamed_chain_str_file, path_to_structures)
